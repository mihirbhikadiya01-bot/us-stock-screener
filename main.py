...
# ---- SHORT-TERM options: 2-4 week swing ideas ----
ST_OUT       = int(os.getenv("ST_OUT", "3"))
ST_DTE_MIN   = int(os.getenv("ST_DTE_MIN", "14"))
ST_DTE_MAX   = int(os.getenv("ST_DTE_MAX", "28"))
ST_ETF_OUT   = int(os.getenv("ST_ETF_OUT", "1"))
ST_ETF_LIST  = [x.strip().upper() for x in
                os.getenv("ST_ETF_LIST", "SPY,QQQ,IWM,GLD,SLV").split(",") if x.strip()]
LEAPS_DAYS   = {int(x) for x in os.getenv("LEAPS_DAYS", "0,3").split(",")
                if x.strip().isdigit()}   # 0=Mon ... 6=Sun; default Mon+Thu

# ---- CHEAP PLAYS: contracts costing <= $3.00 premium ----
CHEAP_MAX_PREM = float(os.getenv("CHEAP_MAX_PREM", "3.0"))
CHEAP_ST_OUT   = int(os.getenv("CHEAP_ST_OUT", "2"))   # 2 short-term cheapies
CHEAP_LONG_OUT = int(os.getenv("CHEAP_LONG_OUT", "1")) # 1 long swing/LEAPS cheapie
...
def send_telegram(text, label="report"):
    ...
    if chunk.strip():
        chunks.append(chunk)
    if len(chunks) > 1:
        tag = label.upper().replace(" REPORT", "")
        chunks = [c if i == 1 else f"⤷ {tag} (part {i}/{len(chunks)})\n{c}"
...
def _bs_price(S, K, T, iv, r=0.04, call=True):
    """Black-Scholes European price. T in years, iv as decimal."""
    try:
        if not S or not K or not iv or iv <= 0 or T <= 0:
            return None
...
def option_stop_estimate(prem, K, iv_dec, dte, stock_stop, bull):
    """Estimated contract value if the stock hits the stock-stop soon
    (~1/3 of DTE elapsed, same IV). Mechanical estimate, not a quote."""
    try:
        if not prem or prem <= 0 or not stock_stop or not K:
            return None, None
        if not iv_dec or iv_dec <= 0:
            return None, -50.0          # fallback: common -50% premium rule
        T = max((dte or 30) - min(14, (dte or 30) / 3.0), 5) / 365.0
        v = _bs_price(stock_stop, K, T, iv_dec, call=bull)
        if v is None:
            return None, -50.0
        return v, (v - prem) / prem * 100.0
    except Exception:
        return None, None
...
def analyze_option(d, force_direction=None, dmin=None, dmax=None, target=60):
    """Pull the option chain for one shortlisted name and build a mechanical
    swing-trade idea (default 45-90 DTE; pass dmin/dmax for other windows)."""
    ...
    if not price or not atr or atr <= 0:
        return None
    try:
        best, best_dte, df = get_chain(d["ticker"], direction, dmin, dmax, target)
    except Exception:
        return None
    ...
    try:
        ...
        if prem and prem > 0 and K:
            ...
            t1_pl, t1_pct = _pl(t1)
            t2_pl, t2_pct = _pl(t2)
            sv, sp = option_stop_estimate(prem, K, bal.get("iv"), best_dte, stop, bull)
            est = {"cost": cost, "t1_pl": t1_pl, "t1_pct": t1_pct,
                   "t2_pl": t2_pl, "t2_pct": t2_pct,
                   "stop_val": sv, "stop_pct": sp}
    except Exception:
        est = None
...
def build_options_watchlist(scored):
    ...
    # GUARANTEE a SPY/QQQ/IWM read even when their chains fail: direction note
    have = {o["ticker"].upper() for o in etf_final}
    for t in ETF_CORE:
        if t in have or t not in by_ticker:
            continue
        d = by_ticker[t]
        kgs, direction = kgs_score(d, force_direction=(
            "bullish" if (d.get("trend_stack") or 0) >= 2 else "bearish"))
        etf_final.append({"ticker": t, "name": d.get("name", t), "kind": "ETF",
                          "direction": direction, "data_only": True,
                          "price": d.get("price"), "kgs": kgs,
                          "trend_stack": d.get("trend_stack"),
                          "conviction": kgs, "trust": trust_tier(kgs)})

    return final[:OPT_OUT], len(shortlist) + len(etf_shortlist), shortlist, etf_final


# ===========================================================================
# SHORT-TERM — 2-4 week options on the strongest current setups
# ===========================================================================
def build_short_term(scored):
    if ST_OUT <= 0:
        return []
    by_ticker = {d["ticker"].upper(): d for d in scored}
    cands = []
    for d in scored:
        if d.get("is_etf") or str(d["ticker"]).upper().endswith(".TO"):
            continue
        if (d.get("price") or 0) < OPT_MIN_PRICE:
            continue
        if (d.get("avg_vol") or 0) < OPT_MIN_AVGVOL:
            continue
        sc, direction, _ = options_setup_score(d)
        if direction != "neutral" and sc > 0:
            cands.append((sc, d))
    cands.sort(key=lambda x: x[0], reverse=True)
    out = []
    for _, d in cands[:max(ST_OUT * 2, 6)]:
        print(f"    [short] {d['ticker']} ...")
        try:
            idea = analyze_option(d, dmin=ST_DTE_MIN, dmax=ST_DTE_MAX, target=21)
        except Exception:
            idea = None
        if idea:
            idea["kind"] = "SHORT"
    ...
    # +1 short-term ETF from the dedicated list (best setup wins)
    etf_ideas = []
    for t in ST_ETF_LIST:
        d = by_ticker.get(t)
        if not d:
            continue
        force = None
        if kgs_score(d)[1] == "neutral":
            force = "bullish" if (d.get("trend_stack") or 0) >= 2 else "bearish"
        try:
            idea = analyze_option(d, force_direction=force,
                                  dmin=ST_DTE_MIN, dmax=ST_DTE_MAX, target=21)
        except Exception:
            idea = None
        if idea:
            idea["kind"] = "SHORT-ETF"
            etf_ideas.append(idea)
        time.sleep(REQUEST_DELAY_SECONDS)
    etf_ideas.sort(key=lambda x: x["conviction"], reverse=True)
    return out + etf_ideas[:ST_ETF_OUT]


# ===========================================================================
# CHEAP PLAYS — contracts <= $3.00 premium (lotto-risk profile)
# ===========================================================================
def _cheap_pick(idea):
    """Choose the best contract <= CHEAP_MAX_PREM (prefer closest to money)."""
    for key in ("bal", "aggr", "safe"):
        sd = idea.get(key)
        p = (sd or {}).get("prem")
        if p and 0.05 <= p <= CHEAP_MAX_PREM:
            return key, sd
    return None, None


def build_cheap_plays(scored):
    if CHEAP_ST_OUT + CHEAP_LONG_OUT <= 0:
        return []
    cands = []
    for d in scored:
        if str(d["ticker"]).upper().endswith(".TO"):
    ...
    def hunt(n, dmin, dmax, target, kind):
        out = []
        for _, d in cands[:14]:
            if len(out) >= n:
                break
            print(f"    [cheap {kind}] {d['ticker']} ...")
            try:
                idea = analyze_option(d, dmin=dmin, dmax=dmax, target=target)
            except Exception:
                idea = None
            if not idea:
                continue
            key, sd = _cheap_pick(idea)
            if not sd:
                continue
            idea["kind"] = kind
            idea["cheap_note"] = (f"💸 Featured: {sd['strike']:.0f} @ ${sd['prem']:.2f} "
                                  f"(${sd['prem']*100:,.0f}/contract)")
            out.append(idea)
            time.sleep(REQUEST_DELAY_SECONDS)
        return out

    shorts = hunt(CHEAP_ST_OUT, ST_DTE_MIN, ST_DTE_MAX, 21, "CHEAP-2W")
    longs = hunt(CHEAP_LONG_OUT, 45, 400, 90, "CHEAP-LONG")
    return shorts + longs
...
def analyze_leaps(d):
    ...
    try:
        ...
        if prem and prem > 0 and K:
            ...
            sv, sp = option_stop_estimate(prem, K, bal.get("iv"), dte, stop, True)
...
def _opt_block(o):
    kindtag = o.get("kind", "SWING")
    side = "📈 CALL" if o["direction"] == "bullish" else "📉 PUT"
    e = o.get("entry") or (None, None)

    # Direction-read fallback (no usable option chain, e.g. core ETF data gap)
    if o.get("data_only"):
        return "\n".join([
            "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄",
            f"#{o['_rank']} {kindtag} {side}  {o['ticker']} — {o['name'][:20]}",
            f"Direction read only (option data unavailable)",
            f"💲 {_px(o.get('price'))} · KGS {o.get('kgs', '–')}/100 · Trend {o.get('trend_stack', '–')}/4",
        ])
    ...
    def stk(sd):
        if not sd:
            return "–"
        p = sd.get("prem")
        return f"{sd['strike']:.0f}" + (f"@${p:.2f}" if p else "")
    ...
    if o.get("iv"):
        ...
        iv_bits.append(f"IV {o['iv']:.0f}%{tag}")
    if o.get("iv_rank") is not None:
        iv_bits.append(f"IVR {o['iv_rank']}")
    iv_bits.append(f"OI {int(o.get('oi') or 0):,}")

    rr = f" · R/R {o['rr']:.1f}" if o.get("rr") else ""
    est = o.get("est") or {}
    L = ["┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄",
         f"#{o['_rank']} {kindtag} {side}  {o['ticker']} — {o['name'][:20]}",
         f"{o.get('trust', '–')} {o['conviction']}/100 · {o.get('setup', '')}",
         f"🧩 {' · '.join((o.get('matches') or [])[:3])}",
         f"💲 {_px(o['price'])} · 🎯 In: {_px(e[0])}–{_px(e[1])} · ⛳ {_px(o.get('breakout'))}",
         f"🏁 {_px(o.get('t1'))} → {_px(o.get('t2'))}{rr}",
         f"📅 {o['exp']} ({o['dte']}d) · 🎟 {stk(o['safe'])} / {stk(o['bal'])} / {stk(o['aggr'])}"]
    # Stops: stock level + estimated option premium at that level
    stop_line = f"🛑 Stock {_px(o.get('stop'))}"
    if est.get("stop_pct") is not None:
        if est.get("stop_val") is not None:
            stop_line += f" → option ≈ ${est['stop_val'] * 100:,.0f} ({est['stop_pct']:+.0f}%)"
        else:
            stop_line += f" → option rule: exit at {est['stop_pct']:.0f}% premium"
    L.insert(6, stop_line)
    if est.get("cost"):
        L.append(f"💵 ${est['cost']:,.0f}/ct → T1 {_money(est['t1_pl'])} ({est['t1_pct']:+.0f}%)"
                 f" · T2 {_money(est['t2_pl'])} ({est['t2_pct']:+.0f}%) · max −100%")
    L.append("📊 " + " · ".join(iv_bits))
    if o.get("cheap_note"):
        L.append(o["cheap_note"])
    flags = []
    if o.get("uoa") is not None and o["uoa"] >= 0.8:
        flags.append("🔥 unusual flow")
    if not o.get("liquid"):
        flags.append("⚠ thin liquidity")
    if o.get("earnings_before"):
        flags.append("⚠ earnings pre-expiry")
    if flags:
        L.append(" · ".join(flags))
    return "\n".join(L)


def build_options_report(opts, date_str, etf_ideas=None, leaps=None, shorts=None, cheapies=None):
    ...
    if shorts:
        for i, o in enumerate(shorts, 1):
            o["_rank"] = i
        L += [f"\n{DIV}", "⚡  SHORT-TERM (2–4 WK) — fast theta, strict stops", DIV]
        L += [_opt_block(o) for o in shorts]
    if cheapies:
        for i, o in enumerate(cheapies, 1):
            o["_rank"] = i
        L += [f"\n{DIV}", f"💸  CHEAP PLAYS (≤ ${CHEAP_MAX_PREM:.0f} contracts) — lotto risk, tiny size", DIV]
        L += [_opt_block(o) for o in cheapies]
    if leaps:
...
def run_longterm():
    ...
    date_str = dt.datetime.now(ET).strftime("%d %b %Y · %I:%M %p ET")
    print("=" * 60)
    ...
    shorts = build_short_term(scored)
    cheapies = build_cheap_plays(scored)
    leaps = []   # LEAPS go out Mon+Thu in market hours (tradeable), not Saturday
    ...
    rpt_opt = build_options_report(opts, date_str, etf_ideas, leaps, shorts, cheapies)
...
def run_options():
    ...
    shorts = build_short_term(scored)
    cheapies = build_cheap_plays(scored)
    leaps = build_leaps(scored) if now.weekday() in LEAPS_DAYS else []
...
def run_live():
    ...
    heartbeat = os.getenv("LIVE_HEARTBEAT_RUN", "false").lower() in ("1", "true", "yes")
    print(f"[LIVE] {now_et:%Y-%m-%d %I:%M %p ET} — watchlist trigger check")
    wl = load_watchlist()
    if not wl:
        print("  No watchlist yet (runs after the first Saturday/options scan).")
        if heartbeat:
            send_telegram(f"🔎 Live check {now_et:%I:%M %p ET}\n"
                          "No watchlist yet — run the Options Scan once to build it.",
                          "live status")
        return
    ...
    if alerts:
        msg = "\n".join([DIV, "⚡ LIVE TRIGGER ALERTS", f"🗓 {now_et:%d %b %Y · %I:%M %p ET}", DIV, ""]
                        + alerts
        ...
        if heartbeat:
            send_telegram(f"✅ Live check {now_et:%I:%M %p ET} — {len(wl)} names watched, "
                          "no fresh triggers right now. System working.", "live status")
